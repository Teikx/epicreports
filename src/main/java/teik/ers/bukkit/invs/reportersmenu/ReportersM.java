package teik.ers.bukkit.invs.reportersmenu;
...
import teik.ers.bukkit.configs.inventories.ReportersMenu;
...
public class ReportersM {
    ...
    public ReportersM(InventoryMG inventoryMG) {
        ...
        this.reportersMenu = inventoryMG.configInvsMG.getReportersMenu();
    }


    public void createReportersMenu(InventoryPlayer inventoryPlayer){
        ...
        Inventory inventory = Bukkit.createInventory(null, 54, reportersMenu.getMenuTitle());
        List<Report> reportersReports = invHelperMG.getAllReportersList(filterType, serverName);

        int totalPages = invsUtils.calculateTotalPages(reportersReports.size());
        ...
        invsUtils.setChest(inventory, page, totalPages, filterType, 49,
                reportersMenu.getChestTitle(),
                reportersMenu.getChestLore()
        );

        SortReportersList(reportersReports, sortType);
        fillReportersInventorySlots(inventory, reportersReports, page);

        player.openInventory(inventory);

        invDataUMG.putPlayerSortTypeReporters(player, sortType);
        invDataUMG.putPlayerFilterTypeReporters(player, filterType);
        invDataMG.putPerPlayerReporterList(player, reportersReports);
    ...
    private void SortReportersList(List<Report> sortedList, SortType sortType) {
    ...
    private void fillReportersInventorySlots(Inventory inv, List<Report> reportedsObj, int page) {
        ...
        for (int i = start; i < end; i++) {
            ...
            String title = reportersMenu
                    .getReporterNameTagTitle().replaceAll("%ReporterName%", report.getReporterName());
            List<String> lore = new ArrayList<>(reportersMenu.getReporterNameTagLore());

            ReplaceReporterLore(lore, report);
    ...
    private void ReplaceReporterLore(List<String> lore, Report report){
